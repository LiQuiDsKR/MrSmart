...
@Deprecated("old")
class BluetoothManager_Old (private val context: Context, private val activity: Activity) {
        ...
        for (standby in standbyList) {
            when (standby.second.type) {
                "RENTALREQUEST" -> {
                    try {
                        isReady = false
                        requestData(RENTAL_REQUEST_SHEET_FORM_STANDBY, standby.second.json, object :
                ...
                "RENTAL" -> {
                    isReady = false
                    try {
                        requestData(RENTAL_REQUEST_SHEET_APPROVE_STANDBY, standby.second.json, object:
                            BluetoothManager_Old.RequestCallback{
                            override fun onSuccess(result: String, type: Type) {
                                Log.d("standby",result)
                                try {
                ...
                "RETURN" -> {
                    isReady = false
                    try {
                        requestData(RETURN_SHEET_FORM_STANDBY, standby.second.json, object:
                            BluetoothManager_Old.RequestCallback{
                            override fun onSuccess(result: String, type: Type) {
                                Log.d("standby",result)
                                try {
