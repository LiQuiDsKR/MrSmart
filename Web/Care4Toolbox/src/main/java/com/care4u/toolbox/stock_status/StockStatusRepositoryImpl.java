...
public class StockStatusRepositoryImpl implements StockStatusRepositoryCustom {
	  private final JPAQueryFactory queryFactory;
		...
		@Override
		public List<StockStatusSummaryByMainGroupDto> getStockStatusSummaryByMainGroupDto(long toolboxId, LocalDate currentDate) {
			QStockStatus stockStatus = QStockStatus.stockStatus;
			...
			List<Tuple> results = queryFactory
			    .select(stockStatus.toolbox.name,
			    		...
			    		stockStatus.goodCount.sum(),
			    		stockStatus.damageCount.sum()
			    			.add(stockStatus.faultCount.sum()
			    		)
			    )
			    .from(stockStatus)
			    .leftJoin(stockStatus.tool, tool)
			    .leftJoin(tool.subGroup, subGroup)
			    .leftJoin(subGroup.mainGroup, mainGroup)
			    .where(
		                stockStatus.toolbox.id.eq(toolboxId)
		                .and(stockStatus.currentDay.eq(currentDate))
			    )
			    .groupBy(mainGroup.id, mainGroup.name)
			    .fetch();
			...
			List<StockStatusSummaryByMainGroupDto> dtoList= results.stream()
				    .map(tuple -> new StockStatusSummaryByMainGroupDto(
						...
						tuple.get(stockStatus.damageCount.sum()
							.add(stockStatus.faultCount.sum())
						))
					)
		...
		public List<StockStatusSummaryByMainGroupDto> getStockStatusSummaryByMainGroupDtoWithMonth(long toolboxId,
			...
			List<Tuple> results = queryFactory
			    ...
			    .from(stockStatus)
			    .where(stockStatus.currentDay.in(JPAExpressions
			    		.select(stockStatus)
					    .from(stockStatus)
					    .leftJoin(stockStatus.tool, tool)
					    .groupBy(tool.subGroup.id, stockStatus.currentDay.month())
					    .select(stockStatus.currentDay.max())))
			    .leftJoin(stockStatus.tool, tool)
			    .leftJoin(tool.subGroup, subGroup)
			    .leftJoin(subGroup.mainGroup, mainGroup)
			    .groupBy(mainGroup.id, mainGroup.name, stockStatus.currentDay.month())
			    .fetch();
